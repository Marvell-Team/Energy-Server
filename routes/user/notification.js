...
router.get("/:idUser", middle , async function (req, res, next) {
  ...
  if(idUser!==null){
    const noti = await notificationModel.find({id_user:idUser}).populate({path:'id_billdetail',populate :{path : 'products.id_product',populate :{path : 'id_image'}}}).populate('id_user');
